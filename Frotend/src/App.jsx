...
const App = () => {
  ...
  const handleAddTask = async () => {
    if (task.trim()) {
      try {
        const res = await axios.post('http://localhost:5000/api/tasks', { task, completed });
  ...
  const editTaskHandler = async () => {
    try {
      const res = await axios.put(`http://localhost:5000/api/tasks/${id}`, { task, completed });
      setTasks(tasks.map((item) => (item._id === id ? { ...item, task: res.data.task } : item)));
    } catch (error) {
